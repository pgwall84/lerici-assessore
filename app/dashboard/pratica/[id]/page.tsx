...
export default function PraticaPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  function apriWhatsApp() {
    ...
    const righe = [
      ...
      `📊 Stato: ${STATO_LABEL[pratica.stato]}${pratica.priorita === "URGENTE" ? " 🔴" : ""}`,
    ];
  ...
  return (
    <div className="space-y-4 pb-8">
      ...
      <div className="flex flex-wrap gap-2">
        ...
        {pratica.priorita === "URGENTE" && (
          <span className="text-xs px-2.5 py-1 rounded-full bg-red-100 text-red-700 font-medium">🔴 Urgente</span>
        )}
      </div>
      ...
      {pratica.segnalante && (
        <div className="bg-white rounded-xl border border-gray-200 p-4 text-sm">
          ...
          {pratica.segnalante.email && <p className="text-gray-500">{pratica.segnalante.email}</p>}
        </div>
      )}
      ...
      {/* Note */}
      <div className="bg-white rounded-xl border border-gray-200 p-4">
        <p className="font-medium text-gray-700 mb-3 text-sm">Note</p>
        <div className="space-y-2 mb-3">
          {pratica.note.length === 0 && (
            <p className="text-xs text-gray-400">Nessuna nota</p>
          )}
          {pratica.note.map(n => (
