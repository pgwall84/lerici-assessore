...
**Implementato (2026-09-21)**: riconoscimento per indirizzo esatto di Questura, Carabinieri (4 stazioni/comandi), Vigili del Fuoco (3 caselle) e ISA 10 — tabella `ENTI_ENTRATA` in `lib/classificatore.ts` (`enteEntrataPerIndirizzo`), chiamata in `classificaESalva` dopo i Gestori. **Automatico** (richiesta di Marco 2026-09-21: estendere l'automatismo il più possibile), come ANCI/Regione/Governo: categoria `ENTE:<nome esatto EnteVario>` → Progetto (Attività) sotto l'ente via `eseguiProgettoVarie`, etichetta `Istituzioni/<ente>` (`gestoreAutomaticoEnte` in `lib/import-automatico.ts`, prefisso gestito in cron e conferma). Per aggiungere un indirizzo/ente: una riga in `ENTI_ENTRATA` (+ il nodo nell'albero se l'ente è nuovo nel seed, per la pre-selezione nel form). Estesi anche gli indirizzi reali di ANCI (`pec.anci.it`, `anciliguria.eu`, `anciliguria@pec.it`) e Governo (Prefettura `pec.interno.it`, PNRR Istruzione) in `categoriaVariaPerDominio`. Rischio noto: alcune mail ANCI/Ministeri sono newsletter e creeranno un Progetto (eliminabile) — da rivalutare se il rumore diventa alto.
...
**REVISIONE (2026-09-21, Marco) — sostituisce la decisione qui sotto per le mail in entrata dei gestori**: una mail mandata da un gestore non è una contestazione (le contestazioni le apre il Comune; il gestore al massimo risponde, e la risposta si aggancia alla contestazione già aperta via continuazione forte). Quindi le mail che arrivano da un indirizzo di gestore noto ricevono **solo l'etichetta `Gestori/<nome>`**, nessuna Contestazione creata (`eseguiMailGestore`, `lib/import-automatico.ts`). Il principio "sempre un'entità" resta valido per le altre categorie. Le righe già in coda scansionate prima dell'aggancio 6.2 sono state riclassificate con uno script una tantum.
...
## 12. Data di ricezione e "Altre deleghe" (albero Gmail `Varie/<nome>`) (2026-09-22/23, richieste di Marco)
