 ...
 * indirizzo mittente esatto (categoriaGestoreEntrataPerIndirizzo in lib/classificatore.ts).
 * Decisione di Marco (2026-09-21, sostituisce quella del 2026-09-15 che creava una Contestazione):
 * una mail mandata da un gestore NON è una contestazione — le contestazioni le apre il Comune, il
 * gestore al massimo risponde, e quella risposta si aggancia alla contestazione già tracciata
 * (trovaContinuazioneForte, prima di questa funzione). Qui quindi nessuna entità: solo il
 * completamento della riga; l'etichetta Gmail "Gestori/<nome>" la applica il chiamante da
 * etichettaPerCategoria(categoria).
 */
export async function eseguiMailGestore(): Promise<EsitoEsecuzione> {
